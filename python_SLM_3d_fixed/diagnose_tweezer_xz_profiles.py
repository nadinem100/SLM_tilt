...
# Use path relative to this script's location, not the current working directory
...
for tw in tweezer_positions:
    ix, iy, x_um, y_um = tw
    # Round y to nearest 10 um to group rows
...
if len(y_groups) > 0:
    ...
    print(f"  Analyzing {len(selected_tweezers)} tweezers from same row (y ~ {best_row_key*10:.0f} um)")
else:
...
print(f"\nScanning z = {z_scan.min():.1f} to {z_scan.max():.1f} um ({Z_STEPS} steps)...")
...
for tw_idx, (ix, iy, x_um, y_um) in enumerate(selected_tweezers):
    ...
    print(f"  Tweezer {tw_idx+1} @ ({x_um:+6.1f}, {y_um:+6.1f}) um:")
    print(f"    Peak at z = {peak_z:+6.1f} um, sharpness = {sharpness:.2f}")
...
ax_xy.set_xlabel('x [um]', fontsize=13, fontweight='bold')
ax_xy.set_ylabel('y [um]', fontsize=13, fontweight='bold')
...
if hasattr(bundle, 'z_per_spot') and bundle.z_per_spot is not None:
    ...
    print(f"  Z range: [{z_min_theory:.1f}, {z_max_theory:.1f}] um")
else:
...
print(f"  Z positions: {z_positions[0]:.1f}, {z_positions[1]:.1f}, {z_positions[2]:.1f} um")
