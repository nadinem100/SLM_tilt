...
# Can be overridden via command-line arg 5
TILT_ANGLE_X = float(sys.argv[5]) if len(sys.argv) > 5 else -13  # degrees
...
def main():
    ...
    print("="*70)
    print("ADAPTIVE GS - MATLAB HORIZONTAL SPACING (80x80)")
    print("="*70)
    ...
    # ========== SAVE RESULTS ==========
    print("\n--- Saving results ---")
    label = f"_matlab_horiz_{N_HORIZ}x{N_VERT}_sp{spacing_um:.1f}um_tilt{int(TILT_ANGLE_X)}deg_{N_Z_PLANES}planes_{ITERATIONS}iter_scal{SCAL}_waist{WAIST_COEFF}"
    ...
    print("DONE!")
    print("="*70)
    print(f"\nConfiguration: {N_HORIZ}x{N_VERT}, MATLAB spacing={spacing_um:.1f}um, tilt={TILT_ANGLE_X}deg, {ITERATIONS} iter, {N_Z_PLANES} z-planes")
