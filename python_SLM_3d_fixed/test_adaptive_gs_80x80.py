"""
Test the adaptive multi-plane GS algorithm with FIXED correction.
Uses corrected algorithm that preserves geometric z-targets.

Configuration: 80x80 grid with 7.2um spacing
"""
...
# Grid configuration - 80x80 with 7.2um spacing
N_HORIZ = 80
N_VERT = 80
SPACING_UM = 7.2  # um spacing between tweezers
...
TILT_ANGLE_X = int(sys.argv[5]) if len(sys.argv) > 5 else -13  # Default -13 degrees
WAIST_UM = WAIST_COEFF / 2 * 1e3  # microns

# Adaptive parameters
Z_SCAN_EVERY = 5  # Scan every 5 iterations
Z_SCAN_RANGE_UM = 50.0  # +/-50 um around target
...
def save_phase_bmp(phase: np.ndarray, out_path: Path) -> None:
    """Save phase mask as 8-bit BMP (0-255 maps to 0-2pi)."""
...
def main():
    ...
    print("="*70)
    print("ADAPTIVE GS TEST - FIXED CORRECTION (80x80, 7.2um spacing)")
    print("="*70)

    OUT_DIR = Path("slm_output_paraxial/adaptive_test_fixed")
    ...
    print(f"\nConfiguration: {N_HORIZ}x{N_VERT} grid, {SPACING_UM}um spacing, {ITERATIONS} iterations, {N_Z_PLANES} z-planes")
    print("="*70)
