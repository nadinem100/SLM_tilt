...
class SLMTweezers:
    def __init__(self, yaml_path: str | Path, *, redSLM: int = 1, scal: int | float = 1) -> None:
        ...
        # Stored pixel offsets for correct physical position calculation
        self._target_cols_ref: Optional[np.ndarray] = None  # pixel offsets in x (columns)
        self._target_rows_ref: Optional[np.ndarray] = None  # pixel offsets in y (rows)
        self._valid_mask: Optional[np.ndarray] = None       # which tweezers are in bounds
    ...
    def set_optics(self, *, wavelength_um: float, focal_length_um: float) -> None:
        ...
        And compute correct physical target positions from stored pixel offsets.
        """
        ...
        # --- Compute correct physical target positions from pixel offsets ---
        # The FFT relates SLM plane to focal plane via:
        #   x_focal = (λ * f) / (N * pixel_pitch) * pixel_index
        # where N is the array size and pixel_index is offset from center
        if self._target_cols_ref is not None and self._target_rows_ref is not None:
            # Focal plane pixel size in µm
            focal_pixel_x_um = (self.wavelength_um * self.focal_length_um) / (W * pu)
            focal_pixel_y_um = (self.wavelength_um * self.focal_length_um) / (H * pu)
            
            # Convert pixel offsets to physical positions in focal plane
            x_um_all = self._target_cols_ref * focal_pixel_x_um
            y_um_all = self._target_rows_ref * focal_pixel_y_um
            
            # Apply the valid mask (tweezers that are in bounds)
            x_um_valid = x_um_all[self._valid_mask]
            y_um_valid = y_um_all[self._valid_mask]
            
            # Center the positions (in case of any asymmetry)
            x_um_valid = x_um_valid - np.mean(x_um_valid)
            y_um_valid = y_um_valid - np.mean(y_um_valid)
            
            # Store as (K, 2) array: columns [x_um, y_um]
            self.target_xy_um = np.stack([x_um_valid, y_um_valid], axis=1).astype(np.float32)
            
            # Print diagnostic
            print(f"  [set_optics] Focal plane pixel size: {focal_pixel_x_um:.3f} x {focal_pixel_y_um:.3f} µm")
            print(f"  [set_optics] Physical target range: x=[{np.min(x_um_valid):.1f}, {np.max(x_um_valid):.1f}] µm, "
                  f"y=[{np.min(y_um_valid):.1f}, {np.max(y_um_valid):.1f}] µm")
    ...
    def set_target_grid(self, *, n_horiz: int, n_vert: int, spacing_um: float, odd_tw: int = 1,
        ...
        # --- Store pixel offsets for correct physical position calculation in set_optics() ---
        # target_cols_ref and target_rows_ref are pixel offsets from the center of the FFT output
        self._target_cols_ref = target_cols_ref.copy()  # All tweezers (before valid filter)
        self._target_rows_ref = target_rows_ref.copy()
        self._valid_mask = valid.copy()
        
        # Placeholder for target_xy_um - will be computed correctly in set_optics()
        # For now, set to None to indicate it needs to be computed
        self.target_xy_um = None
