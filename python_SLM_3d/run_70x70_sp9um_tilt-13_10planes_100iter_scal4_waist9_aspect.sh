#!/bin/bash
#SBATCH --job-name=slm_70x70_9um_10p_asp
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
sed -i 's/N_HORIZ = .*/N_HORIZ = 70/' test_adaptive_gs_temp.py
sed -i 's/N_VERT = .*/N_VERT = 70/' test_adaptive_gs_temp.py
sed -i 's/SPACING_UM = .*/SPACING_UM = 9.0/' test_adaptive_gs_temp.py
...
echo "Grid: 70x70, Spacing: 9.0um (aspect corrected), Tilt: -13deg"
