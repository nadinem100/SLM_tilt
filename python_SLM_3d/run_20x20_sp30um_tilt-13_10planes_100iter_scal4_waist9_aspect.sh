#!/bin/bash
#SBATCH --job-name=slm_20x20_30um_10p_asp
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
# Copy the aspect corrected file and modify parameters
cp test_adaptive_gs_aspect_corrected.py test_adaptive_gs_temp.py

# Modify parameters using sed
sed -i 's/N_HORIZ = .*/N_HORIZ = 20/' test_adaptive_gs_temp.py
sed -i 's/N_VERT = .*/N_VERT = 20/' test_adaptive_gs_temp.py
sed -i 's/SPACING_UM = .*/SPACING_UM = 30.0/' test_adaptive_gs_temp.py
...
echo "Starting SLM simulation (aspect corrected)..."
echo "Grid: 20x20, Spacing: 30.0um (aspect corrected), Tilt: -13deg"
