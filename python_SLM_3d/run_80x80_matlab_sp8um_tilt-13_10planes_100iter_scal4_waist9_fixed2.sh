#!/bin/bash
#SBATCH --job-name=slm_80x80_sp8_10p_f2
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
# Copy the matlab spacing file from fixed2 folder and modify parameters
cp fixed2/test_adaptive_gs_matlab_spacing.py test_adaptive_gs_temp.py
...
sed -i 's/SPACING_FACTOR = .*/SPACING_FACTOR = 4.0/' test_adaptive_gs_temp.py
...
echo "Starting SLM simulation (fixed2)..."
echo "Grid: 80x80, MATLAB spacing_factor=4.0 (~8um), Tilt: -13deg"
