...
def compute_focal_plane_intensity(
    ...
    # EXACT defocus formula: φ(X,Y) = -k*z*R²/(2f(f-z))
    # This matches the GS algorithm in slm_tweezers_class assign_planes_from_tilt()
    denom = 2.0 * f * (f - z_um)
    if abs(denom) < 1e-6:
        # Guard against z ≈ f
        denom = 2.0 * f * f
...
def plot_xz_slice(
    ...
    """
    Create X-Z slice figure with tilt overlay.
    ...
    im = ax.imshow(xz_intensity, aspect='auto', origin='lower',
                   extent=extent, vmin=vmin, vmax=vmax, cmap='hot')
    
    ax.set_xlabel('x [µm]', fontsize=13)
    ax.set_ylabel('z [µm]', fontsize=13)
    ax.set_title(f'X–Z slice (y-band center={y_center_px}, ±{y_band_halfwidth_px}px, {agg_mode})',
