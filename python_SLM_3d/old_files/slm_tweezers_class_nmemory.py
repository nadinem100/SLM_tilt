...
"""
Stateful, class‑based refactor scaffold for the SLM tweezers code.

Goal: keep functionality identical while turning the script into a single
stateful object with:
  • a clear initialization phase (loads YAML, sets sizes/dtypes, allocates arrays)
  • explicit configuration and cached derived values
  • methods that mutate internal state instead of returning huge tuples

This file is a *drop‑in architecture* you can paste into your repo and then
...
well‑labeled TODO hooks where you can paste/port your existing math (GS loop,
...
You can now call follow‑ups like `slm.set_target_grid(...)` and `slm.run_gs(...)`
without re‑loading YAML or re‑allocating base arrays.
"""
...
class SLMTweezers:
    ...
    def set_optics(self, *, wavelength_um: float, focal_length_um: float) -> None:
        ...
        Also precompute pupil coordinate grids X^2, Y^2, X*Y (µm^2).
        """
        ...
        self._k_over_2f2 = float(k / (2.0 * self.focal_length_um ** 2))
    ...
    def set_tilt_plane(self, *, tilt_x_deg: float = 0.0, tilt_y_deg: float = 0.0) -> None:
        ...
        z_i = tx * (xy[:, 0] - x0) + ty * (xy[:, 1] - y0)  # µm of defocus per spot
    ...
    def _phi_quad_for_spot(self, i: int) -> np.ndarray:
        ...
        φ_i(X,Y) = (k/2f^2)*(α_i X^2 + β_i Y^2 + γ_i X Y).
        """
    ...
    def set_target_grid(self, *, n_horiz: int, n_vert: int, spacing_um: float, odd_tw: int = 1,
        ...
        Rotation removed: lattice is axis‑aligned. Spacing matches legacy FFT-based
        ...
        # --- NEW: physical target positions in the focal plane (µm), centered ---
        ...
        # Center just in case n_horiz/n_vert are even and rounding produced off-by-½ issues
    ...
    # -------- GS loop & output --------
    def run_gs(self, *, iterations: int, Gg: float, useFilter: bool = False) -> None:
        ...
        # Optionally build a gentle filter
        if useFilter:
        ...
        # Crop back to reduced grid center
        scal = int(self.config.scal)
    ...
    def _gs_loop(self, *, A_in: np.ndarray, pad: np.ndarray, A_target: np.ndarray, height_corr2: np.ndarray,
                 ...
                 coordinates: np.ndarray, box1: int) -> np.ndarray:
        """
        Port of the legacy GS loop with per-tweezer weighting.
        """
        g = np.ones_like(A_target, dtype=A_target.dtype)
        B_target = np.abs(A_target)  # Use magnitude

        # Initialize psi with random phase in center
        ...
        onesmatrix = np.ones_like(height_corr2)
        error_signal = 1.0
        ...
        for ii in range(1, iterations + 1):
            ...
            # Apply feedback correction BEFORE weight calculation
            if ii > 1 and coordinates.size:
                correction_denom = onesmatrix - Gg * (onesmatrix - np.sqrt(height_corr2))
                correction_denom[correction_denom == 0] = 1.0
                B_out_flat[coordinates] = B_out_flat[coordinates] / correction_denom.ravel()

            # Calculate weights for uniformity
            if coordinates.size:
                B_out0 = B_out_flat[coordinates]
                B_mean = float(np.mean(B_out0))

                # Reshape: (num_tweezers, block) then transpose to (block, num_tweezers)
                B_out0_reshaped = B_out0.reshape(num_tweezers, block).T
                B_out_box = np.mean(B_out0_reshaped, axis=0)  # Mean over pixels in each tweezer

                # Weight: boost dim tweezers
            ...
            # Backward propagation with weighted constraint
    ...
    def assign_planes_from_tilt(self, *, tilt_x_deg: float = 0.0, tilt_y_deg: float = 0.0,
                                z_min_um: float = -50.0, z_max_um: float = 50.0, n_planes: int = 5) -> None:
        """
        Compute per-spot defocus z_i from a desired tilt and assign each tweezer
        to the nearest of 'n_planes' evenly-spaced z-planes in [z_min_um, z_max_um].
        """
        assert self.target_xy_um is not None, "Call set_target_grid() first."
        ...
        # center xy for nice symmetry
        xy = self.target_xy_um.astype(np.float64)
        x0, y0 = float(np.mean(xy[:, 0])), float(np.mean(xy[:, 1]))
        z_i = tx * (xy[:, 0] - x0) + ty * (xy[:, 1] - y0)  # µm defocus per tweezer (from tilt)

        # build plane grid
        self._z_planes = np.linspace(float(z_min_um), float(z_max_um), int(n_planes)).astype(np.float32)
        # assign each tweezer to the nearest plane index
        idx = np.abs(self._z_planes[None, :] - z_i[:, None]).argmin(axis=1).astype(np.int32)

        # members[p] = list of tweezer indices s that live in plane p
        self._members = [np.where(idx == p)[0].tolist() for p in range(n_planes)]

        # remember z per tweezer (for debugging/plots)
        self._z_per_spot = z_i.astype(np.float32)

        # precompute per-plane Fresnel propagation phase φ_p(X,Y)
        # CRITICAL FIX: Use Fresnel propagation φ = (k/2z)(X²+Y²), NOT thin-lens defocus
        # For tilted planes, we're propagating to different distances, not defocusing a lens
        assert self._pupil_X2 is not None and self._pupil_Y2 is not None
        assert hasattr(self, 'wavelength_um'), "Call set_optics() first to set wavelength"
        k = 2.0 * np.pi / self.wavelength_um

        self._phi_planes = []
        for z_p in self._z_planes:
            if abs(z_p) < 1e-6:  # handle focal plane (z=0) specially
                phi_p = np.zeros_like(self._pupil_X2, dtype=np.float32)
            else:
                # Fresnel propagation phase: φ = (k/2z)(X²+Y²)
                # This is the correct free-space propagator for distance z
                phi_p = (k / (2.0 * float(z_p))) * (self._pupil_X2 + self._pupil_Y2)
            self._phi_planes.append(phi_p.astype(np.float32, copy=False))

    def run_gs_multiplane_v3(self, *, iterations: int = 50, Gg: float = 0.6, verbose: bool = True) -> None:
        """
        Corrected multi-plane GS algorithm.

        Key insight: Each plane should have its own target amplitude pattern,
        and back-propagated fields should be properly combined.
        """
        ...
        # Global gain and target
        g = np.ones_like(self.A_target, dtype=self.A_target.dtype)
        B_target_global = np.abs(self.A_target)

        # Create per-plane target amplitude patterns
        # Each plane only has non-zero targets for its assigned tweezers
        B_target_planes = []
        for p in range(P):
        ...
        onesmatrix = np.ones_like(self.height_corr2)
        error_signal = 1.0
        ...
        for ii in range(1, iterations + 1):
            ...
            # Accumulate back-propagated fields from all planes
            A_new_accum = np.zeros_like(A_mod, dtype=np.complex128)

            # Track output amplitudes across all planes
            B_out_all = np.zeros_like(B_target_global, dtype=np.float64)
            ...
            for p in range(P):
                ...
                # === AMPLITUDE CONSTRAINT FOR THIS PLANE ===
                # Key: Use per-plane target (only non-zero for this plane's tweezers)
                A_iter_p = g * B_target_planes[p] * np.exp(1j * psi_out_p)

                # Back-propagate
                if abs(self._z_planes[p]) < 1e-6:
                    A_back_p = fftshift(ifft2(ifftshift(A_iter_p)))
                else:
                    A_back_p = fftshift(ifft2(ifftshift(A_iter_p))) * np.exp(-1j * phi_p)

                # Accumulate (all planes contribute)
                A_new_accum += A_back_p

            # Normalize accumulated field to preserve energy
            # This prevents runaway growth when summing multiple planes
            total_energy_in = np.sum(np.abs(A_mod) ** 2)
            total_energy_out = np.sum(np.abs(A_new_accum) ** 2)
            if total_energy_out > 0:
                A_new_accum *= np.sqrt(total_energy_in / total_energy_out)

            # === FEEDBACK CORRECTION & WEIGHTS ===
            B_out_flat = B_out_all.ravel()

            # Apply feedback correction BEFORE weight calculation
            if ii > 1 and self.coordinates.size:
                correction_denom = onesmatrix - Gg * (onesmatrix - np.sqrt(self.height_corr2))
                correction_denom[correction_denom == 0] = 1.0
                B_out_flat[self.coordinates] = B_out_flat[self.coordinates] / correction_denom.ravel()

            # Calculate weights
            if self.coordinates.size:
                B_out0 = B_out_flat[self.coordinates]
                B_mean = float(np.mean(B_out0[B_out0 > 0]))  # Only consider non-zero pixels
                ...
                if verbose and (ii % 10 == 0 or ii <= 5):
                    print(f'Iteration {ii}: error = {error_signal:.5f}, '
                          f'weight range = [{weight.min():.3f}, {weight.max():.3f}]')
    ...
    def run_gs_multiplane_v3_efficient(self, *, iterations: int = 50, Gg: float = 0.6,
                                       verbose: bool = True, cleanup_every: int = 10) -> None:
        """
        Memory-efficient version of run_gs_multiplane_v3.

        New parameter:
            cleanup_every: Run memory cleanup every N iterations (default: 10)
        """
        from scipy.fft import fft2, ifft2, fftshift, ifftshift
        import numpy as np
        ...
        if verbose:
            print(f"\n{'=' * 70}")
            print(f"Multi-plane GS v3 (Memory-Efficient): {num_tweezers} tweezers, {P} planes")
            print(f"Memory cleanup every {cleanup_every} iterations")
            for p in range(P):
                print(f"  Plane {p} @ z={self._z_planes[p]:+.1f} µm: {len(self._members[p])} tweezers")
            print(f"{'=' * 70}")
        ...
        # Create per-plane target patterns (these are kept in memory)
        B_target_planes = []
        for p in range(P):
            B_target_p = np.zeros_like(B_target_global, dtype=np.float32)
            if len(self._members[p]) > 0:
        ...
        A_mod = np.zeros_like(self.A_target, dtype=np.complex64)
        A_new_accum = np.zeros_like(A_mod, dtype=np.complex128)
        B_out_all = np.zeros_like(B_target_global, dtype=np.float64)

        # Main loop
        for ii in range(1, iterations + 1):
            # Reuse A_mod instead of creating new array
            np.multiply(self.A_in.astype(np.complex64, copy=False),
                        np.exp(1j * psi), out=A_mod)
            ...
            for p in range(P):
                ...
                del A_out_p, psi_out_p, B_out_p, A_iter_p, A_back_p, B_out_flat
            ...
            if ii % cleanup_every == 0:
                gc.collect()

        # Final cleanup of pre-allocated arrays
        del A_mod, A_new_accum, B_out_all
...
#     slm = SLMTweezers(yaml_path="../slm_parameters.yml", redSLM=1, scal=2)
#     slm.init_fields(waist_um=9/2*1e3)
#     nhoriz = 80; nvert = 80;
#     slm.set_target_grid(n_horiz=nhoriz, n_vert=nvert, spacing_um=7.2, odd_tw=1, box1=1)
#     slm.run_gs(iterations=20, Gg=0.6, useFilter=False)
#     bundle = slm.save_pickle(label=f"_{nhoriz}x{nvert}_demo")
#     print("Saved:", bundle.file)from __future__ import annotations
