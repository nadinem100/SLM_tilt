...
class SLMTweezers:
    ...
    def _gs_loop(self, *, A_in: np.ndarray, pad: np.ndarray, A_target: np.ndarray, height_corr2: np.ndarray,
        ...
        for ii in range(1, iterations + 1):
            ...
            if coordinates.size:
                ...
                g_flat = g.ravel()
                g_flat[coordinates] = weight_expanded
    ...
    def assign_planes_from_tilt(self, *, tilt_x_deg: float = 0.0, tilt_y_deg: float = 0.0,
        ...
        z_i = tx * (xy[:, 0] - x0) + ty * (xy[:, 1] - y0)  # Âµm defocus per tweezer (from tilt)
        ...
        # precompute per-plane Fresnel propagation phase Ï†_p(X,Y)
        # CRITICAL FIX: Use Fresnel propagation Ï† = (k/2z)(XÂ²+YÂ²), NOT thin-lens defocus
        ...
        for z_p in self._z_planes:
            if abs(z_p) < 1e-6:  # handle focal plane (z=0) specially
                ...
                # Fresnel propagation phase: Ï† = (k/2z)(XÂ²+YÂ²)
...
#             self._phi_planes.append(phi_p.astype(np.float32, copy=False))
#
#     def run_gs_multiplane(self, *, iterations: int = 50, Gg: float = 0.6, verbose: bool = True) -> None:
#         """
#         Multi-plane GS that EXACTLY mirrors run_gs structure.
#
#         The ONLY difference from standard GS:
#         - Standard: pupil ->[FFT]-> focal -> constraint
#         - Multi-plane: pupil ->[exp(iÏ†_p)FFT]-> focal_p -> constraint -> accumulate
#
#         Everything else (weights, feedback, phase update) is IDENTICAL.
#         """
#         from scipy.fft import fft2, ifft2, fftshift, ifftshift
#
#         assert self.A_in is not None and self.pad is not None
...
#
#         g = np.ones_like(self.A_target, dtype=self.A_target.dtype)
#         B_target = np.abs(self.A_target)
#
#         # Initialize phase (identical to run_gs)
#         psi = np.zeros_like(self.A_target, dtype=np.float32)
#         my = (psi.shape[0] - self.psi0.shape[0]) // 2
#         mx = (psi.shape[1] - self.psi0.shape[1]) // 2
#         psi[my:my + self.psi0.shape[0], mx:mx + self.psi0.shape[1]] = self.psi0
...
#
#         if verbose:
#             print(f"\nMulti-plane GS: {num_tweezers} tweezers, {P} planes")
#
#         # Main loop
#         for ii in range(1, iterations + 1):
#             # === FORWARD PROPAGATION (multi-plane modification) ===
#             A_mod = self.A_in.astype(np.complex64, copy=False) * np.exp(1j * psi)
#
#             # Accumulate back-propagated fields from all planes
#             A_new_accum = np.zeros_like(A_mod, dtype=np.complex128)
#
#             # Track output amplitudes for weight calculation
#             B_out_all = np.zeros_like(B_target, dtype=np.float64)
#
#             # Process each plane
#             for p in range(P):
#                 # Forward to plane p
...
#                     A_out_p = fftshift(fft2(ifftshift(A_mod)))
#                 else:
#                     # zâ‰ 0: apply Fresnel phase, then FFT
#                     A_out_p = fftshift(fft2(ifftshift(A_mod * np.exp(1j * phi_p))))
#
#                 psi_out_p = np.mod(np.angle(A_out_p), 2 * np.pi)
#                 B_out_p = np.abs(A_out_p).astype(np.float64, copy=False)
#
#                 # Store amplitudes (only for THIS plane's tweezers)
#                 if len(self._members[p]) > 0:
#                     B_out_flat = B_out_p.ravel()
#                     for s in self._members[p]:
#                         idx0 = s * block
#                         idxs = self.coordinates[idx0:idx0 + block]
#                         B_out_all.ravel()[idxs] = B_out_flat[idxs]
#
#                 # === AMPLITUDE CONSTRAINT (identical to run_gs) ===
#                 # Key: do EXACTLY what standard GS does
#                 A_iter_p = g * B_target * np.exp(1j * psi_out_p)
#
#                 # Back-propagate
#                 if abs(self._z_planes[p]) < 1e-6:
#                     A_back_p = fftshift(ifft2(ifftshift(A_iter_p)))
...
#                 A_new_accum += A_back_p
#
#             # === FEEDBACK CORRECTION & WEIGHTS (identical to run_gs) ===
#             B_out_flat = B_out_all.ravel()
#
#             # Apply feedback correction BEFORE weight calculation
#             if ii > 1 and self.coordinates.size:
...
#                 g = g_flat.reshape(g.shape)
#
#             # === UPDATE PHASE (from accumulated field) ===
#             psi = np.mod(np.angle(A_new_accum), 2 * np.pi).astype(np.float32)
#             psi *= self.pad
#
#         # Extract final phase mask (identical to run_gs)
#         scal = int(self.config.scal)
#         ypix, xpix = self.A_in.shape[0] // scal, self.A_in.shape[1] // scal
#         mid_y_start = int((scal - 1) * ypix / 2)
#         mid_y_end = int((scal + 1) * ypix / 2)
...
#
#         if verbose:
#             print(f"âœ“ Convergence: {self.convergence}")
#
#     def save_pickle(self, *, out_dir: str | Path = "/Users/nadinemeister/PyCharmMiscProject/python_SLM_3d/out",
#                     label: Optional[str] = None) -> ResultBundle:
#         assert self.phase_mask is not None and self.tweezlist is not None and self.tweezer_mask is not None
