...
goal_plane_tilt = 1; %degrees
theta = goal_plane_tilt * pi/180; %radians now
...
%[XX,YY] = meshgrid(x,y);
k = 2*pi/(689*10^(-9));
z_compensation = xpp * tan(theta); 
zzz = repmat(z_compensation, size(XX, 1), 1);
...
imagesc(abs(A_out).^2);
colorbar;
title('Final Tweezer Field Intensity');
xlabel('Pixel');
ylabel('Pixel');
axis image;


filename = sprintf('try20znew1_n%dx%d_iter%d_spac%d_angle%d', n_horiz, n_vert, iterations, spacing_factor, goal_plane_tilt);
Copy_of_save_mask(phase_mask, filename, x_pixels, y_pixels, scal)